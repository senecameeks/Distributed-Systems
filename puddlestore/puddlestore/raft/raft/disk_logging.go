...
// ReadStableState reads the stable sto re log file (or the backup if that fails),
...
// For snapshotting:
// // TruncateLogReverse removes the log entries up to and including index from the log cache log file.
// func TruncateLogReverse(raftLogFd *FileData, index uint64) error {
//
// 	fileSizeBeforeIndex, exist := raftLogFd.idxMap[index]
//
// 	entireFileSize, exist := raftLogFd.idxMap[len(raftLogFd.idxMap)]
//
// 	if !exist {
// 		return fmt.Errorf("Truncation failed, log index %v doesn't exist\n", index)
// 	}
